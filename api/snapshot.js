...
/* 12 was a listing-exchange tag in `idx`, since removed with the Nasdaq-listed
   and NYSE-listed filters it fed. The NUMBER stays at 12 rather than rolling
   back: it is a cache key, not a version of the format, and dropping it to 11
   would invalidate the blobs the crons have already written — forcing a core
   recompute that spends FMP quota, and putting the extended tier back to
   "pending" until 22:20 UTC — all to purge a field nothing reads. */
const SCHEMA = 12;
